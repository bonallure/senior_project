...
class DoctorPrescriptions extends React.Component{
    ...
    async getPrescriptions() {
        ...
        const url = "http://localhost:8080/doctor/prescriptions/" + this.state.user.id;
    ...
    async getPatientPrescriptions() {
        console.log(this.state)
        const url = "http://localhost:8080/doctor/prescriptions/patient/" + this.patientId;
        const response = await axios.get(url);
        try {
            console.log("this is the response", response.data);
    ...
    onClick = (e) => {
        e.preventDefault();
        this.props.history.push("/doctor/prescription/new")
    }
